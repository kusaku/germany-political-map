...
A single-page, static WebGL atlas of party polling, state-election results and Bundestag results since reunification across Germany. Political data is bundled into the repository and refreshed daily.
...
- Daily polling, latest state-election and Bundestag-history views share the same interactive scene.
...
- State election results: [Federal Returning Officer](https://www.bundeswahlleiterin.de/en/service/landtagswahlen.html). The provisional 2026 results for Berlin, Mecklenburg-Vorpommern and Sachsen-Anhalt come from their respective official state result portals.
- Bundestag results: official [Federal Returning Officer results](https://www.bundeswahlleiterin.de/bundestagswahlen/2025/ergebnisse.html), stored in `data/bundestag-results.json`. It covers the 1990–2025 elections and uses second votes (party-list votes), the decisive party vote.
...
The historical archive is immutable and only needs rebuilding when its parser changes:

```sh
node scripts/build-bundestag-history.mjs
```
