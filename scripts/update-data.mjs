...
// The Federal Returning Officer's state-election pages lag these newly held
// elections. Keep the jurisdictions' published statewide second-vote totals
// authoritative until the federal index catches up.
const SEPTEMBER_2026_RESULTS = {
  "11": {
    date: "2026-09-20",
    turnout: 74.2,
    provisional: true,
    source: "https://www.wahlen-berlin.de/wahlen/BE2026/Afspraes/agh/downloads.html",
    results: [
      ["Left", 25.7], ["CDU", 18.8], ["AfD", 16.3], ["Greens", 14.3], ["SPD", 12.1],
      ["BSW", 4.7], ["FDP", 2.5], ["Tierschutzpartei", 2.1], ["Volt", 2.1],
      ["Die PARTEI", 0.7], ["PdF", 0.3], ["DKP", 0.1], ["Die Urbane", 0.1], ["ÖDP", 0.1]
    ]
  },
  "13": {
    date: "2026-09-20",
    turnout: 78.1,
    provisional: true,
    source: "https://wahlen.mvnet.de/dateien/ergebnisse.2026/landtagswahl/html/l_99.html",
    results: [
      ["AfD", 38.2], ["SPD", 35.5], ["Left", 6.5], ["Greens", 5.7], ["CDU", 4.9],
      ["BSW", 4], ["Tierschutzpartei", 1.1], ["FDP", 1], ["Handwerker Partei Deutschland", 0.5],
      ["Free Voters", 0.4], ["Die PARTEI", 0.3], ["Team Freiheit", 0.3], ["PIRATEN", 0.2],
      ["Volt", 0.2], ["Bündnis C", 0.1], ["KPD", 0.1], ["ÖDP", 0.1], ["PdF", 0.1], ["WLD", 0.1]
    ]
  }
};
...
async function updateElectionResults() {
  ...
  for (const [id, result] of Object.entries(SEPTEMBER_2026_RESULTS)) {
    states[id] = { ...result, results: result.results.map(([party, value]) => ({ party, value })).sort(byResult) };
  }
